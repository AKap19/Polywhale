...
async function fetchSportsMarkets() {
  ...
  } catch (e) { console.log("fetchSportsMarkets error:", e.message); }
  const seen = new Set();
...
async function scoreWallet(address) {
  try {
    ...
    if (!r.ok) {
      console.log(`scoreWallet ${address}: HTTP ${r.status}`);
      return null;
    }
    const positions = await r.json();
    if (!Array.isArray(positions)) {
      console.log(`scoreWallet ${address}: not array, got`, typeof positions);
      return null;
    }
    console.log(`scoreWallet ${address}: ${positions.length} positions`);
    const settled = positions.filter(p => p.redeemed === true || p.outcome != null);
    console.log(`scoreWallet ${address}: ${settled.length} settled`);
    ...
    console.log(`scoreWallet error ${address}:`, e.message);
    return null;
  }
}
...
export default async function handler(req, res) {
  const MIN_SIZE     = parseInt(process.env.MIN_SIZE      || "500");
