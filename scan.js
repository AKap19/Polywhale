// scan.js - PolyWhale Scanner
// Put this in the /api/ folder on GitHub
    ...
    fields: [
      {
        ...
        value: [
          ...
          `**Implied prob:** ${(price * 100).toFixed(1)}%`,
          `**Line move:** ${lineMove}`,
        ].join("\n"),
        inline: true,
      },
      {
        ...
        value: [
          ...
          `**ROI:** ${roiStr}`,
          `**Volume:** $${Math.round(wallet.totalVolume).toLocaleString()}`,
          `\`${walletShort}\``,
        ].join("\n"),
        inline: true,
      },
      {
        ...
        value: [
          ...
          `[Wallet History](https://polymarket.com/profile/${wallet.address})`,
          `**Category:** ${tags}`,
        ].join("  ·  "),
        inline: false,
      },
...
export default async function handler(req, res) {
  ...
  try {
    ...
    console.log(`[polywhale] ${markets.length} markets`);
